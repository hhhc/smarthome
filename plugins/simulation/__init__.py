...
# Callback. Writes the event to the file
    ...
    def update_item(self, item, caller=None, source=None, dest=None):
        if (item.conf['sim'] == 'track') and (self.state()==2 or self.state()==3):
            now = self._sh.now()
            day=now.day
...
#----------------------- _schedule_recording_start ---------------------------
    def _schedule_recording_start(self,time):
        ...
        logger.info('Scheduling record start {}'.format(time))
        self._sh.scheduler.add('startrecord', self._start_recording, next=time)

#----------------------- _schedule_recording_stop ----------------------------
#    def _schedule_recording_stop(self):
#        self.state(3,'Simulation')
#        self._sh.scheduler.remove('startrecord')
#        next = self._sh.now()
#        next += timedelta(days=1)
#        next = next.replace(hour=0, minute=0,second=0)
#        self._message_item('Recording will stop 00:00 next night', caller='Simulation') 
#        logger.info('Scheduling record stop {}'.format(next))
#        self._sh.scheduler.add('startrecord', self._stop_recording,  next=next)
...
# schedules it. 
    ...
    def _set_item(self, **kwargs):
        ...
        if target!= None and value!= None:
            logger.info('Setting {} to {}'.format(target,value))
        ...
        if entry !='':
            ...
            if (self.lastday != '') and (self.lastday != day):
                logger.info('Found next day {} {} {} shitfing to tomorrow.'.format(target, value, next))
                next=next+timedelta(1)
            self._message_item('Next event: {}<br>{}   {}'.format(time,target,value,'Simulation'))
            logger.info('Scheduling {} {} {}'.format(target, value, next))
...
#-------------------------------- do_nothing ----------------------------------
    def _do_nothing(self):
        logger.info('Do nothing state: {} control: {}'.format(self.state(), self.control()))
